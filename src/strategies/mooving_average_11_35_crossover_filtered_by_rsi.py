...
def executar(ativo, tempo_grafico, raw_dir, indicators_dir, strategies_dir):
    ...
    nome_estrategia = "CROSSOVER_MA_11_35_RSI_14"
    ...
    arq_ma11 = os.path.join(indicators_dir, f"{ativo}_{tempo_grafico}_MA_11.csv")
    arq_ma35 = os.path.join(indicators_dir, f"{ativo}_{tempo_grafico}_MA_35.csv")  # Corrigido para 35
    ...
    df["MA_11"] = df_ma11["MA_11"]
    df["MA_35"] = df_ma35["MA_35"]
    ...
    df["Cruzou_Acima"] = (df["MA_11"] > df["MA_35"]) & (df["MA_11"].shift(1) <= df["MA_35"].shift(1))
    df["Cruzou_Abaixo"] = (df["MA_11"] < df["MA_35"]) & (df["MA_11"].shift(1) >= df["MA_35"].shift(1))
